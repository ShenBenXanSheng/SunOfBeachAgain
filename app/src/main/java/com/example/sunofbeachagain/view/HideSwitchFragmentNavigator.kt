...
public open class HideSwitchFragmentNavigator(
    ...
    private fun navigate(
        ...
        //如果已经添加了，直接显示
        if (frag == null) {
            println(" create new fragment...$tag")
        ...
        for (fragment in fragmentManager.getFragments()) {
            println("hide fragment -- > " + fragment.javaClass.name)
            ft.hide(fragment)
        }
        if (!frag.isAdded) {
            println("add fragment ... $tag")
        ...
        val initNavigation: Boolean = backStack.isEmpty()
        // TODO Build first class singleTop behavior for fragments
        val isSingleTopReplacement = (
                navOptions != null && !initNavigation &&
    ...
    public constructor(fragmentNavigator: Navigator<out FragmentNavigator.Destination>) :
